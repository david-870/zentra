...
export function fallbackAnswer(
  ...
  if (intent === "recommend") {
    ...
    if (focus === "website") {
      return {
        text: `If you need to be found and give people a way to reach you, *Starter* is the usual starting point — ${starterPrice()}. If enquiries already come in but follow-up is messy, *Standard* — ${growthPrice()}.\n\nWhat's the actual problem — no site yet, or people find you and then it goes quiet?`,
        handoff: false,
        qualify: false,
      };
    }
    if (focus === "automation" || focus === "whatsapp" || focus === "ai_assistant" || focus === "crm") {
      return {
        text: `For messages, follow-up and less manual work, *Standard* is the usual starting point — ${growthPrice()}.\n\nWhat still has to be done by hand every day?`,
        handoff: false,
        qualify: false,
      };
    }
  ...
  if (intent === "pricing") {
    ...
    if (focus === "automation" || focus === "whatsapp" || focus === "ai_assistant" || focus === "crm") {
      return {
        text: `When the job is messages, follow-up and less manual work, *Standard* is the usual starting point — ${growthPrice()}. That is a starting price. Custom automation is confirmed after we understand the workflow.\n\nWhat still has to be done by hand every day?`,
        handoff: false,
        qualify: false,
      };
    }
