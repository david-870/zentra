...
export function capabilityReply(id: CapabilityId): string {
  switch (id) {
    ...
    case "login_portal":
      return `Yes. A login, membership area or customer portal is a *web app* — people use it, they don't just read pages. That is closer to custom work (Scale is the starting point, from ₦1,500,000, confirmed after we talk).\n\nWho would log in — customers, staff, or both?`;
    case "dashboard":
    ...
    case "analytics":
      return `Yes. We can set the site up so you can see how people find you and what they do there. Growth includes that kind of setup.\n\nDo you already have Google Analytics, or would this be new?`;
    case "crm_connect":
      return `Yes. A customer system can sit with the website or WhatsApp so a new enquiry lands in one list instead of a chat pile. Connecting tools you already use is typical of Scale-level work.\n\nWhat do you use for customers today — WhatsApp, a spreadsheet, or something else?`;
    case "custom_system":
      return `Yes, custom business software is something we can discuss. We don't guess the features — we look at the workflow, then build around it. Starting point is Scale, from ₦1,500,000, confirmed after a conversation.\n\nWhat are you currently using to manage that process, and what should the new system actually do?`;
    case "mobile_native":
