...
const INTENT_HINTS: { intent: AdaIntent; pattern: RegExp }[] = [
  ...
  { intent: "packages", pattern: /\b(package|starter|growth|scale|standard|complete|business package|custom package)\b/i },
...
export function topicFromText(text: string): LeadContext["lastTopic"] {
  ...
  if (/package|starter|growth|scale|standard|complete|business package|custom package/.test(value)) return "packages";
  return undefined;
}
